...
public class ExtractionIncremental extends AbstractExtraction {
    protected static Logger cis_logger = LoggerFactory.getInstance("cis_extraction.ExtractionIncremental");
    ...
    public void init(Date kickoff, Long repeat, Hashtable args) {
        ...
        if(cis_logger.isDebugEnabled()) {
            ...
            cis_logger.debug(" - jndiCSIDataSourceName: " + m_jndiCSIDataSourceName);
    ...
    public void run(Notification notification, Hashtable args) {
        ...
        try {
            synchronized (this) {
                cisRun(CISConstants.CIS_EXTRACTTYPE_WEEK);
            }
        } catch (Exception e) {
            cis_logger.fatal(e);
        }
